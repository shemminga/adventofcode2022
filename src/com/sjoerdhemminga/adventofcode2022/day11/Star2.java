...
public final class Star2 {
    ...
    private void doFile(final String filename) throws IOException, URISyntaxException {
        ...
        try (final Stream<String> lines = Files.lines(Paths.get(input.toURI()))) {
            ...
            final long[] activityCounters = new long[monkeys.length];

            //dumpItemsAndActivityCounters("Star 2 At start", monkeys, activityCounters);
            //System.out.println("=".repeat(100));

            final long maxModulo = Arrays.stream(monkeys)
                    .mapToLong(Monkey::divisableByTest)
                    .reduce(1, (x, y) -> x * y);
            System.out.println("maxModulo = " + maxModulo);

            for (int round = 1; round <= 10000; round++) {
                playRound(monkeys, activityCounters, maxModulo);

                if (round == 1 || round == 20 || round % 1000 == 0)
                    dumpActivityCounters("After round " + round, activityCounters);
                //dumpItemsAndActivityCounters("Star 2 After round " + round, monkeys, activityCounters);
            }

            final long[] sortedActivityCounters = Arrays.stream(activityCounters)
                    .sorted()
                    .toArray();

            final long monkeyBusinessLevel = sortedActivityCounters[sortedActivityCounters.length - 1] *
    ...
    private void dumpItemsAndActivityCounters(final String header, final Monkey[] monkeys,
            final long[] activityCounters) {
        System.out.println();
    ...
    private void dumpActivityCounters(final String header, final long[] activityCounters) {
        System.out.println();
        System.out.println();
        System.out.println(header);
        System.out.println("-".repeat(header.length()));

        for (int i = 0; i < activityCounters.length; i++) {
            System.out.printf("Monkey %d inspected items %3d times.%n", i, activityCounters[i]);
        }
    }

    private void playRound(final Monkey[] monkeys, final long[] activityCounters, final long maxModulo) {
        for (int i = 0; i < monkeys.length; i++) {
            while (monkeys[i].hasItemToThrow()) {
                activityCounters[i]++;
                final long itemReadyToThrow = monkeys[i].getItemReadyToThrowStar2(maxModulo);
                final int target = monkeys[i].getTarget(itemReadyToThrow);
                monkeys[target].items().addLast(itemReadyToThrow);
                //System.out.printf("S2 %2d -> %2d %5d%n", i, target, itemReadyToThrow);
            }
        }
    }

    private static Monkey[] parse(final Deque<String> lines) {
